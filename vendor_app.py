...
st.markdown("""
    ...
    div[data-testid="stButton"] button {
        ...
        height: 40px !important;      /* 降低高度，不要那麼笨重 */
        font-size: 16px !important;   /* 字體適中 */
    ...
    div[data-testid="stButton"] button[kind="primary"] {
        ...
        color: #000 !important;
        box-shadow: 0 2px 5px rgba(0,204,150,0.3); /* 加一點陰影更有質感 */
    }
    div[data-testid="stButton"] button[kind="primary"]:hover {
        background-color: #00A87D !important;
    }

    /* 取消按鈕：改為「幽靈按鈕」(透明底+灰框)，視覺上退後一步 */
...
with tab1:
    if not pending:
        ...
        st.write("") # 空一行，不要用分隔線，讓畫面乾淨點

        selected_p = []
        for o in pending:
            with st.container(border=True):
                c_sel, c_info = st.columns([1, 9]) # 勾選框欄位窄一點
        ...
        if selected_p:
            # 使用醒目的提示區塊
            st.markdown(f"""
            <div style="background-color:#00332a; padding:10px; border-radius:8px; border:1px solid #00CC96; margin-top:15px; margin-bottom:10px;">
...
with tab2:
    if not working:
        ...
        if selected_w_data:
            st.markdown(f"""
            <div style="background-color:#3e2723; padding:10px; border-radius:8px; border:1px solid #FFAB91; margin-top:20px; margin-bottom:20px;">
            ...
            for item in selected_w_data:
                ...
                with st.container():
                    ...
                    <div class="report-card">
                        <h4 style="color:#00FFCC!important; margin:0;">📄 {item['customer_wo']}</h4>
                        <p style="margin:0; font-size:14px; color:#AAA;">機種：{item['customer_model']} | <b>總數：{qty}</b></p>
                    </div>
                    """, unsafe_allow_html=True)
